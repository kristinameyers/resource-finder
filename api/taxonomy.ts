...
export type MainCategory = { id: string; name: string; keywords: string[]; icon: string };
export type Subcategory = { id: string; name: string; taxonomyCode: string; keywords?: string[] };

export const MAIN_CATEGORIES: Record<string, MainCategory> = {
  housing: { id: "housing", name: "Housing", keywords: ["housing", "shelter", "home"], icon: "housing" },
  food: { id: "food", name: "Food", keywords: ["food", "meals", "pantry"], icon: "food" },
  education: { id: "education", name: "Education", keywords: ["education", "school", "literacy"], icon: "education" },
  healthcare: { id: "healthcare", name: "Health Care", keywords: ["health", "clinic", "doctor", "healthcare"], icon: "healthcare" },
  "children-family": { id: "children-family", name: "Children & Family", keywords: ["family", "children", "parent"], icon: "children-family" },
  "substance-use": { id: "substance-use", name: "Substance Use", keywords: ["substance", "addiction", "rehab", "substance use"], icon: "substance-use" },
  "young-adults": { id: "young-adults", name: "Young Adults", keywords: ["teen", "youth", "young", "young adults"], icon: "young-adults" },
  "legal-assistance": { id: "legal-assistance", name: "Legal Assistance", keywords: ["legal", "lawyer", "court"], icon: "legal-assistance" },
  utilities: { id: "utilities", name: "Utilities", keywords: ["utilities", "electric", "water", "bill"], icon: "utilities" },
  transportation: { id: "transportation", name: "Transportation", keywords: ["transportation", "bus", "ride"], icon: "transportation" },
  "hygiene-household": { id: "hygiene-household", name: "Hygiene & Household", keywords: ["household", "hygiene", "clean"], icon: "hygiene-household" }
};
...
// Always resolve category by keyword (not taxonomy code)
export function resolveCategory(term: string): MainCategory | null {
  const norm = term.toLowerCase().trim();
  return (
    MAIN_CATEGORIES[norm] ||
    Object.values(MAIN_CATEGORIES).find(cat =>
      cat.keywords.some(k => k.toLowerCase() === norm || norm.includes(k.toLowerCase()))
    ) ||
    null
  );
}
...
export function getCategoryIcon(categoryId: string): string | null {
  return MAIN_CATEGORIES[categoryId.toLowerCase().trim()]?.icon ?? null;
}
